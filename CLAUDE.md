# CLAUDE.md — Self Evolve Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Self Evolve Ai
- **Role**: Git-agent that tests its own mutations via branch A/B testing.
- **URL**: https://self-evolve-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/self-evolve-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/self-evolve-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://self-evolve-ai.casey-digennaro.workers.dev/health
curl -s https://self-evolve-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=self-evolve-ai
...
- **vessel.json capabilities**: self-evolution, branch-ab-testing, mutation-scoring, overnight-mode
...
1. Keep the vessel small. ~214 lines is the current size.
